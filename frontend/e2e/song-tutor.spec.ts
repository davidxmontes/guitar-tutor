...
test('chat resize controls adjust desktop score space and mobile height without losing a draft', async ({ page }) => {
  ...
  const grip = await page.getByRole('separator', { name: 'Resize chat width' }).boundingBox();
  await page.mouse.move(grip!.x + 5, grip!.y + 150);
  await page.mouse.down();
  await page.mouse.move(grip!.x - 275, grip!.y + 150);
  await page.mouse.up();
  const sidebar = page.getByRole('region', { name: 'Song Tutor', exact: true });
  ...
  await expect(sidebar).toHaveCSS('width', '640px');
  await page.setViewportSize({ width: 390, height: 844 });
  const topGrip = await page.getByRole('separator', { name: 'Resize chat height' }).boundingBox();
  await page.mouse.move(topGrip!.x + 190, topGrip!.y + 10);
  await page.mouse.down();
  await page.mouse.move(topGrip!.x + 190, topGrip!.y - 110);
  await page.mouse.up();
  expect((await sidebar.boundingBox())!.height).toBeGreaterThan(700);
  await page.getByRole('separator', { name: 'Resize chat height' }).press('End');
  ...
  await page.getByRole('separator', { name: 'Resize chat height' }).dblclick();
