...
export function VoicingComparison({ active, comparison, focus }: {
  ...
  return (
    <section data-testid="voicing-fretboard" aria-label="Voicing relationship fretboard" className="space-y-2">
      ...
      <p className="text-xs">● Active · ◇ {comparison?.label ?? 'Choose a candidate to compare'}{focus ? ' · Tutor focus' : ''}</p>
          ...
          {groups.map(group => <g key={group.role} data-role={group.role}>
            {group.notes.filter(p => p.string >= 1 && p.string <= 6 && p.fret >= 0 && p.fret <= 36).map((p, i) => {
              ...
              return <g key={i}>
                ...
                {group.role === 'comparison' ? <path d={`M${x} ${y-14} l14 14 l-14 14 l-14 -14 Z`} fill="none" stroke="#7dd3fc" strokeWidth="3" /> : <circle cx={x} cy={y} r="11" fill={group.color} />}
