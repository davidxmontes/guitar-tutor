...
import type { Artifact, ArtifactRevision, LibraryItem, V2Session } from '../types/v2';
...
const names = { song_study: 'Song', progression: 'Progression', concept_study: 'Study', exercise: 'Exercise' };
...
export function MyStuff({ onOpen }: { onOpen: (session: V2Session) => void }) {
  ...
  return <section className="my-5 space-y-3" aria-labelledby="my-stuff-heading">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 id="my-stuff-heading" className="text-lg font-bold">My Stuff</h2><p className="text-sm text-[var(--text-secondary)]">Saved music and studies. Open with a fresh conversation.</p></div>
    ...
    {items?.length === 0 && <p className="text-sm text-[var(--text-secondary)]">Save a song, progression, study or exercise to find it here.</p>}
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{items?.filter(item => filter === 'all' || item.kind === filter).map(item => {
      ...
      return <article key={item.id} data-testid="library-item" className="min-w-0 rounded-xl border border-[var(--border-primary)] bg-[var(--card-bg)] p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2"><button type="button" aria-label={`Open ${item.title}`} className={button} disabled={busy} onClick={() => act(async () => onOpen(await apiClient.openLibraryArtifact(item.id)))}>Open</button>
