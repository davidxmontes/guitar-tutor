  ...
  <div hidden={Boolean(preview)} className="lg:flex lg:items-start lg:gap-5">
   <div className="min-w-0 space-y-5 lg:flex-1">
    <header className="space-y-2 border-b border-[var(--border-primary)] pb-3">
      ...
      <div className="flex flex-wrap items-center gap-2"><button type="button" className={`${controlSm} font-semibold`} disabled={!resolved || locked || Boolean(progression?.steps.some(s => !s.positions.length))} onClick={() => {
      ...
      }}>{playing ? 'Stop playback' : caged ? 'Hear adjacent regions' : progression ? 'Hear progression' : physical ? 'Hear D to G' : 'Hear comparison'}</button>
      {scales.length > 0 && <label className="flex items-center gap-1.5 text-sm">Both roots<select aria-label="Both roots" className={controlSm} disabled={locked} value={scales.every(e => e.root === scales[0].root) ? scales[0].root : ''} onChange={e => editScale(null, { root: e.target.value })}><option value="" disabled>Different roots</option>{roots.map(root => <option key={root}>{root}</option>)}</select></label>}
      ...
      <form className="flex flex-wrap items-center gap-2" onSubmit={event => { event.preventDefault(); void saveStudy(); }}>
        <label className="flex min-w-0 items-center gap-1.5 text-sm">Study name<input required maxLength={120} value={studyName} disabled={locked}
          onChange={event => setStudyName(event.target.value)} className={`${controlSm} min-w-0`} /></label>
        <button className={controlSm} disabled={locked || workspace !== saved.current || !studyName.trim()}>{branch.current_artifact_id ? 'Save version' : 'Save as study'}</button>
        {branch.current_artifact_id && <button type="button" className={controlSm} disabled={locked || workspace !== saved.current || !studyName.trim()} onClick={() => saveStudy(true)}>Save as a new study</button>}
    ...
    {scales.length > 0 && <fieldset disabled={locked}>
      <details className="text-sm"><summary className="cursor-pointer py-1 text-[var(--text-secondary)]">Edit each scale and tuning</summary><div className="flex flex-wrap gap-3 pt-2">{scales.map((entity, index) => <div key={entity.id} className="flex flex-wrap items-center gap-2"><label className="flex items-center gap-1.5">Scale {index + 1} root<select aria-label={`Scale ${index + 1} root`} className={controlSm} value={entity.root} onChange={e => editScale(entity.id, { root: e.target.value })}>{roots.map(root => <option key={root}>{root}</option>)}</select></label><label className="flex items-center gap-1.5">Scale {index + 1} mode<select aria-label={`Scale ${index + 1} mode`} className={`${controlSm} max-w-full`} value={entity.mode} onChange={e => editScale(entity.id, { mode: e.target.value as ScaleMode })}>{modes.map(mode => <option key={mode} value={mode}>{mode.replaceAll('_', ' ')}</option>)}</select></label><button className={controlSm} disabled={workspace.entities.length === 1} onClick={() => removeScale(entity.id)}>Remove scale {index + 1}</button></div>)}</div>
        <label className="mt-2 flex items-center gap-1.5">Tuning<select aria-label="Tuning" className={controlSm} value={JSON.stringify(workspace.tuning)} onChange={e => change({ ...workspace, tuning: JSON.parse(e.target.value) })}><option value="[64,59,55,50,45,40]">Standard</option><option value="[64,59,55,50,45,38]">Drop D</option>{!['[64,59,55,50,45,40]', '[64,59,55,50,45,38]'].includes(JSON.stringify(workspace.tuning)) && <option value={JSON.stringify(workspace.tuning)}>Custom</option>}</select></label>
      </details>
    </fieldset>}
      ...
      return <section key={block.id} aria-label={names[block.kind]} className="cw-block min-w-0 space-y-2 rounded-xl border border-[var(--border-primary)] bg-[var(--card-bg)] p-3" style={{ '--cw-span': placement.span, '--cw-row': placement.row + 1, '--cw-order': placement.priority === 'primary' ? 0 : placement.priority === 'supporting' ? 1 : 2 } as React.CSSProperties}>
        <header className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--border-primary)] pb-2">
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <h3 className="font-bold">{names[block.kind]}</h3>
            <fieldset disabled={locked} className="flex flex-wrap items-center gap-2"><label className="flex items-center gap-1.5">Labels<select className={controlSm} value={block.settings.labels} onChange={e => updateSettings(block, { labels: e.target.value as 'notes' | 'intervals' })}><option value="notes">Notes</option><option value="intervals">Intervals</option></select></label>
              {block.kind === 'fretboard' && !resolved?.caged[block.source_id] && <label className="flex items-center gap-1.5">Frets<select className={controlSm} value={`${block.settings.fret_start}-${block.settings.fret_end}`} onChange={e => { const [fret_start, fret_end] = e.target.value.split('-').map(Number); updateSettings(block, { fret_start, fret_end }); }}>{[...new Set([`${block.settings.fret_start}-${block.settings.fret_end}`, '0-5', '3-8', '5-10', '7-12'])].map(range => <option key={range}>{range}</option>)}</select></label>}
              {(workspace.relations.some(r => r.id === block.source_id) || resolved?.caged[block.source_id]) && <label className="flex items-center gap-1.5"><input type="checkbox" checked={block.settings.shared_only} onChange={e => updateSettings(block, { shared_only: e.target.checked })} />{resolved?.caged[block.source_id] ? 'Shared positions only' : 'Shared notes only'}</label>}
            </fieldset>
          </div>
    ...
    {adding && <fieldset disabled={locked} className="flex flex-wrap items-center gap-2 rounded-lg border border-[var(--border-primary)] p-3 text-sm"><legend className="px-1 text-[var(--text-secondary)]">Add a compatible view</legend>
      <label className="flex items-center gap-1.5">Musical source<select className={controlSm} value={sourceId} onChange={e => setSourceId(e.target.value)}>{[...workspace.relations, ...workspace.entities].map(item => <option key={item.id} value={item.id}>{resolved ? workspaceLabel(item.id, resolved) : item.kind}</option>)}</select></label>
      <label className="flex items-center gap-1.5">View type<select className={controlSm} value={allowedViews.includes(viewKind) ? viewKind : allowedViews[0] ?? ''} onChange={e => setViewKind(e.target.value as WorkspaceBlock['kind'])}>{allowedViews.map(kind => <option key={kind} value={kind}>{names[kind]}</option>)}</select></label>
      <button disabled={!allowedViews.length} className={controlSm} onClick={() => { const id = crypto.randomUUID(); change({ ...workspace, blocks: [...workspace.blocks, { id, kind: allowedViews.includes(viewKind) ? viewKind : allowedViews[0], source_id: sourceId, settings: { pattern: (allowedViews.includes(viewKind) ? viewKind : allowedViews[0]) === 'progression' && source?.kind === 'key' ? 'I-V-vi-IV' : null, labels: 'notes', shared_only: false, fret_start: 0, fret_end: 5 } }], composition: [...workspace.composition, { items: [{ block_id: id, span: 12, priority: 'supporting' }] }] }); setAdding(false); addButton.current?.focus(); }}>Add selected view</button>
    </fieldset>}
   ...
   {tutorOpen && <>
    ...
    <aside aria-label="Tutor"
      ...
      className="fixed inset-x-2 bottom-2 z-40 flex max-h-[82dvh] flex-col rounded-xl bg-[var(--card-bg)] p-2 shadow-lg lg:sticky lg:inset-x-auto lg:bottom-auto lg:top-4 lg:z-auto lg:h-[calc(100dvh-2rem)] lg:w-[340px] lg:shrink-0 lg:self-start lg:rounded-none lg:bg-transparent lg:p-0 lg:shadow-none">
