...
function hasFretboard(composition: Composition): boolean {
  return Object.values(composition.slots).flat().some(block => 'kind' in block ? block.kind === 'fretboard' : hasFretboard(block));
}
...
export function ProgressionWorkspace({ branch, onChange }: { branch: V2Branch; onChange: (branch: V2Branch) => void }) {
  ...
  const composition: Composition = view === 'tutor' ? surface.composition : { pattern: 'hero-with-support', focal: 'hero', slots: {
    hero: [{ kind: 'fretboard' }],
    support: view === 'harmonic-function' ? [{ kind: 'harmonic-function' }, { kind: 'progression-editor' }] : view === 'voice-leading' ? [{ kind: 'voice-leading' }, { kind: 'chord-inspector' }] : [{ kind: 'progression-editor' }, { kind: 'progression-idea-list' }],
  } };
  const selectStep = (step_id: string) => { if (busy) return; compare.clear(); if (!hasFretboard(composition)) setView('fretboard'); void edit({ focus: { kind: 'step', step_id } }); };
  const inspect = (target: ProgressionFocus) => {
    if (busy) return;
    compare.clear(); setView(target.kind === 'transition' ? 'voice-leading' : 'harmonic-function'); void edit({ focus: target });
    requestAnimationFrame(() => document.getElementById('workspace-music')?.scrollIntoView({ block: 'start' }));
  };
  return <section className="learning-workspace progression-workspace" data-testid="progression-workspace" aria-busy={busy}>
    ...
    {idea && data && <ProgressionPractice idea={idea} data={data} selectedId={selectedStep?.id} busy={busy} onSelect={selectStep} practice={{ ...practice, enter: () => { compare.clear(); setView('fretboard'); practice.setAudioMode('both'); practice.enter(); } }} />}
