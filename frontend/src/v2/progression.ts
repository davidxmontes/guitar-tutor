...
export type ProgressionResolved = { steps: (ProgressionStep & { positions: ResolvedNote[]; notes: { note: string; degree: string }[]; function: string | null; voicings: (VoicingValue & { label: string })[] })[]; transitions: { from_step_id: string; to_step_id: string; shared: number[]; removed: number[]; added: number[]; assigned: boolean; movement: { string: number; kind: string; semitones: number | null }[] }[]; key_status: string };
