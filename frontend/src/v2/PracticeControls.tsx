...
export function PracticeControls({ practice: p, available, label, allowFocus = true, guideLabel = 'Synthesized guide · simplified articulation, not the original recording' }: { practice: PracticeState; available: boolean; label: string; allowFocus?: boolean; guideLabel?: string }) {
  ...
  const commitTempo = (input: HTMLInputElement) => {
    const value = input.valueAsNumber;
    const valid = Number.isInteger(value) && value >= 1 && value <= 240;
    if (valid) p.setTempo(value);
    input.value = String(valid ? value : p.tempo);
    setTempoError(!valid);
  };
  ...
  return <section aria-label="Practice controls" className="practice-controls">
    ...
    <div className="practice-transport">
      ...
      <label className="practice-field">Tempo <span><input aria-label="Practice tempo" className="w-20" type="number" min="1" max="240" step="1" key={p.tempo} defaultValue={p.tempo} onBlur={e => commitTempo(e.currentTarget)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.currentTarget.blur(); } }} /> BPM</span></label>
    ...
    {tempoError && <p role="alert">Enter a whole-number tempo from 1 to 240 BPM. The previous tempo was restored.</p>}
