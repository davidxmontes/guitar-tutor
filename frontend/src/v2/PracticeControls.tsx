...
export function PracticeControls({ practice: p, available, label }: { practice: PracticeState; available: boolean; label: string }) {
  ...
  return <section aria-label="Practice controls" className="space-y-3 rounded-xl border p-3 text-sm" style={{ background: 'var(--card-bg)', borderColor: 'var(--border-primary)', color: 'var(--text-primary)' }}>
    <div className="flex flex-wrap items-center gap-3">
      ...
      <span className="text-xs text-[var(--text-secondary)]">{p.guideAvailable ? 'Synthesized guide · simplified articulation, not the original recording' : 'Metronome only · no guide audio'}</span>
    </div>
    <div className="flex flex-wrap items-center gap-3">
