...
export function PracticeControls({ practice: p, available, label }: { practice: PracticeState; available: boolean; label: string }) {
  ...
  return <section aria-label="Practice controls" className="space-y-3 rounded-xl border p-3 text-sm" style={{ background: 'var(--card-bg)', borderColor: 'var(--border-primary)', color: 'var(--text-primary)' }}>
    <div className="flex flex-wrap items-center gap-3">
      ...
      <span className="text-xs text-[var(--text-secondary)]">Metronome only · no guide audio</span>
    </div>
    <div className="flex flex-wrap items-center gap-3">
      ...
      <label className="flex min-h-11 items-center gap-2 text-sm"><input className="accent-[var(--accent-700)]" type="checkbox" checked={p.metronome} onChange={e => p.setMetronome(e.target.checked)} />Metronome</label>
      <button type="button" className={p.focused ? `${button} border-[var(--accent-700)] bg-[var(--accent-50)] text-[var(--accent-900)] hover:bg-[var(--accent-100)]` : secondary} aria-pressed={p.focused} onClick={() => p.setFocused(!p.focused)}>{p.focused ? 'Exit Focus' : 'Focus practice'}</button>
