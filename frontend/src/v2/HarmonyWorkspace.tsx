    ...
    {compare.selection.length >= 2 ? <ComparisonView peers={compare.selection} onClear={compare.clear} renderPeer={(peer, config, nudge) => <>
      ...
      if (block.kind === 'chord-inspector' && chord) return <ChordInspector context="harmony" notes={data.chord_notes} functionLabel={data.function} onExplore={() => void explore(chord)} chord={chord} hasKey={!!state.tonal_center} />;
        ...
        if (block.kind === 'chord-diagram' && chord) {
          ...
          return <section aria-label="Selected chord diagram"><h3>{chord.root} {chord.quality}</h3>{positions.length ? <><PhysicalChordDiagram positions={positions} tuning={state.tuning} label={`${chord.root} ${chord.quality}`} selected={focus.kind === 'voicing' && positions.length === data.voicing_positions.length && positions.every(note => data.voicing_positions.some(other => note.string === other.string && note.fret === other.fret))} disabled={busy} onSelect={() => void edit({ focus: { kind: 'voicing', chord, voicing } })} /><Hear voicing={voicing} /><button className="music-button" disabled={busy} onClick={() => void edit({ pin: { chord, voicing } })}>Pin shape</button><button className="music-button" onClick={() => compare.toggle({ kind: 'voicing', id: JSON.stringify(voicing), label: `${chord.root} ${chord.quality}`, positions, tuning: state.tuning })}>Compare shape</button></> : <p>No playable shape is available for this chord and tuning.</p>}</section>;
        }
