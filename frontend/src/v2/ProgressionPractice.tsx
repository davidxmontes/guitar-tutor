...
export function ProgressionPractice({ idea, data, selectedId, busy, onSelect }: { idea: ProgressionIdea; data: ProgressionResolved; selectedId?: string; busy: boolean; onSelect: (id: string) => void }) {
  ...
  return <section className="learning-progression-practice" aria-label="Play this progression">
    ...
    <nav aria-label="Chords in this progression"><ol ref={sequence} className="learning-chord-strip">{data.steps.map((chord, index) => <li key={chord.id} aria-current={practice.active && current === index ? 'step' : undefined}><button type="button" aria-label={`Chord ${index + 1}: ${chord.root} ${chord.quality}`} aria-pressed={chord.id === selectedId} aria-disabled={busy} onClick={() => { if (!busy) { practice.exit(); onSelect(chord.id); } }} onKeyDown={event => {
